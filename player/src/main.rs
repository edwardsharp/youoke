...
async fn main() {
    ...
    let static_files = warp::fs::dir("./locallibrary");

    let routes = ws_route.or(static_files);

    println!("websocket server running at ws://localhost:3030/ws");
    println!("serving ./locallibrary at http://localhost:3030/");
...
//     std::thread::spawn(|| {
//         // web_view::builder()
//         //       .title("Rust WebSocket UI")
//         //       .content(web_view::Content::Url("http://localhost:3030"))
//         //       .size(800, 600)
//         //       .resizable(true)
//         //       .debug(true)
//         //       .user_data(())
...
//         let _ = Command::new("sh").arg("-c").arg("echo hold on plz").spawn();
//     });
// }
